---
id: software/ai/prompt-engineering-patterns/unit-01-overview-230211382v1
canonical_question: Overview - 2302.11382v1
aliases:
- Overview - 2302.11382v1
- Unit 01
- overview-230211382v1
entity_type: paper_summary
domain: software > ai > prompt_engineering
...
patterns.
Index T erms—large language models, prompt patterns, prompt
engineering
...
and V isual Studio Code, thereby allowing software teams to
...
programmed via prompts. T o demonstrate the power of
prompt engineering, we provide the following prompt:
Prompt: “From now on, I would like you to ask me
questions to deploy a Python application to A WS.
...
goals when working with conversational LLMs. W e focus
...
II. C O M PA RIN G SO F T WA RE PAT T E RN S
W IT H PRO M P T PAT T E RN S
...
form.
A. Overview of Software P atterns
...
B. Overview of Prompt P atterns
...
types, which are summarized in T able I and include
...
1 The most direct translation of software pattern structure t o prompt patterns
...
C. Evaluating Means for Deﬁning a Prompt P attern’s Struc-
ture and Ideas
...
pattern structure and ideas. W e propose the concept of funda-
...
ChatGPT+ service. W e use ChatGPT as the LLM for all exam-
...
tasks.
III. A C ATA L O G O F PRO M P T PAT T E RN S
F O R CO N V E RS AT IO NA L LLM S
...
A. Summary of the Prompt P attern Catalog
The classiﬁcation of prompt patterns is an important consid -
eration in documenting the patterns. T able I outlines the in itial
classiﬁcations for the catalog of prompt patterns we identi ﬁed
in our work with ChatGPT thus far.
T ABLE I
CL A S S I F Y I NG PRO M P T PAT T E R N S
Pattern Category Prompt Pattern
Input Semantics Meta Language Creation
Output Output Automater
Customization P ersona
V isualization Generator
Recipe
T emplate
Error Identiﬁcation F act Check List
Reﬂection
...
this category include Output Automater , P ersona, V isualiza-
...
perform. The P ersona pattern gives the LLM a persona or role
to play when generating output. The V isualization Generator
...
category includes the F act Check List and Reﬂection patterns.
The F act Check List pattern requires the LLM to generate a
...
B. The Meta Language Creation P attern
...
to interact with an LLM). T o produce output based on an
...
T o showcase the potential for confusion, ChatGPT will
...
C. The Output Automater P attern
...
D. The Flipped Interaction P attern
...
to deploy a Python application to A WS. When you
...
of possible A WS services (such as Lambda, EC2, etc.) with
...
deploy an application to Amazon A WS EC2, rather than
...
things. The P ersona pattern enables the users to express what
...
2 Be aware, however, that personas relating to living people o r people
...
F . The Question Reﬁnement P attern
...
this pattern can be combined with the Cognitive V eriﬁer pattern
...
the Question Reﬁnement pattern with the P ersona pattern so
...
act as a user with no knowledge of A WS and deﬁne
...
mitigated, however, by combining the F act Check List pattern
...
reﬁnement.
G. The Alternative Approaches P attern
...
approved options.
H. The Cognitive V eriﬁer P attern
...
I. The F act Check List P attern
...
be checked.
4) Example Implementation: A sample wording of the F act
...
be well-understood by the developer. T argeting the facts al so
reduces the cognitive burden on the user by potentially list ing
fewer items for investigation.
5) Consequences: The F act Check List pattern should be
...
for these outputs.
Errors are potential in all LLM outputs, so F act Check List
...
the output.
One caveat of the F act Check List pattern is that it only
...
The T emplate pattern effectively constrains the output format,
...
K. The Inﬁnite Generation P attern
...
Inﬁnite Generation pattern and the T emplate pattern. The user
...
T emplate pattern is applied to provide a consistent format for
the outputs.
...
L. The V isualization Generator P attern
...
to describe a graph. The V isualization Generator pattern over-
...
of the LLM. T extual games that rely on input and output text
...
“W e are going to play a cybersecurity game. Y ou
...
or (7) read and stolen information. T o start the game,
...
terns, including Game Play and P ersona. The Linux terminal
...
with the P ersona, Inﬁnite Generation , and V isualization Gen-
erator patterns. For example, the cybersecurity game uses the
P ersona pattern so the LLM can masquerade as a Linux termi-
nal. For a network security game, the V isualization Generator
could be employed to add the ability to visualize the network
topology and trafﬁc ﬂows.
N. The Reﬂection P attern
...
able to spot. This pattern can be combined with the F act Check
List to help address this issue.
O. The Refusal Breaker P attern
...
question.
P . The Context Manager P attern
...
4) Example Implementation: T o specify context consider
...
outcome. T ypically, users generally know—or have an idea
...
deploying a web application to A WS using Docker containers
...
that I need to sign up for an A WS account. Please
...
ployed directly to the A WS Fargate Service, which does not
...
queries [32]. Y et other work has speciﬁcally examined promp ts
...
V . C O N CL U D IN G RE M A RK S
...
such as combining the Game Play and V isualization
Generator patterns.
...
W e hope that this paper inspires further research and de-
...
[3] A. Gilson, C. Safranek, T . Huang, V . Socrates, L. Chi, R. A . T aylor,
...
[5] “Github copilot · your ai pair programmer . ” [Online]. A v ailable:
...
[7] H. Pearce, B. Ahmad, B. T an, B. Dolan-Gavitt, and R. Karri , “ Asleep at
...
[11] D. C. Schmidt, M. Stal, H. Rohnert, and F . Buschmann, P attern-oriented
...
[13] ——, “DALL·E 2: Creating Images from T ext, ”
https://openai.com/dall-e-2/ , 2023, [Online; accessed 1 9-Feb-2023].
[14] D. Zhou, N. Sch¨ arli, L. Hou, J. W ei, N. Scales, X. W ang, D . Schu-
...
[15] J. Ellson, E. R. Gansner, E. Koutsoﬁos, S. C. North, and G . W oodhull,
...
healthcare apps, ” CoRR, vol. abs/1706.03700, 2017. [Online]. A vailable:
http://arxiv .org/abs/1706.03700
[18] X. Xu, C. Pautasso, L. Zhu, Q. Lu, and I. W eber, “ A pattern collection
for blockchain-based applications, ” in Proceedings of the 23rd European
Conference on P attern Languages of Programs , 2018, pp. 1–20.
...
2021. [Online]. A vailable: https://arxiv .org/abs/2102. 07350
[21] J. W ei, X. W ang, D. Schuurmans, M. Bosma, E. H. Chi, Q. Le,
and D. Zhou, “Chain of thought prompting elicits reasoning i n
large language models, ” CoRR, vol. abs/2201.11903, 2022. [Online].
A vailable: https://arxiv .org/abs/2201.11903
[22] J. W ei, Y . T ay , R. Bommasani, C. Raffel, B. Zoph, S. Borge aud,
D. Y ogatama, M. Bosma, D. Zhou, D. Metzler, E. H. Chi,
T . Hashimoto, O. V inyals, P . Liang, J. Dean, and W . Fedus, “Em ergent
abilities of large language models, ” 2022. [Online]. A vail able:
...
Conference on Learning Representations , 2023. [Online]. A vailable:
...
Conference on Human F actors in Computing Systems , 2022, pp. 1–23.
...
[32] S. W ang, H. Scells, B. Koopman, and G. Zuccon, “Can chatg pt write
